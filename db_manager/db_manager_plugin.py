...
class DBManagerPlugin:
	...
	def initGui(self):
		...
		self.actionTopoCSVImp = QAction( QIcon(":/db_manager/actions/import"), u"ИМПОРТ ТОПОДАННЫХ PolFunDev", self.iface.mainWindow() )
		self.actionTopoCSVImp.setObjectName("dbTopoCSVImport")
		QObject.connect( self.actionTopoCSVImp, SIGNAL( "triggered()" ), self.runCSVTopoImpTool )
		# Add toolbar button and menu item
		if hasattr( self.iface, 'addDatabaseToolBarIcon' ):
			self.iface.addDatabaseToolBarIcon(self.action)
		else:
			self.iface.addToolBarIcon(self.action)
		if hasattr( self.iface, 'addDatabaseTopoCSVImpIcon' ):
			self.iface.addDatabaseToolBarIcon(self.actionTopoCSVImp)
		else:
			self.iface.addToolBarIcon(self.actionTopoCSVImp)	
