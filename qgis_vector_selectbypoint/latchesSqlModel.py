...
class latchesRelSqlModel(QtSql.QSqlRelationalTableModel):
    def __init__(self, QObject_parent=None):
        super(latchesRelSqlModel, self).__init__(QObject_parent)
    ...
    def setData(self, QModelIndex, p_object, int_role=None):
        ...
        if (QModelIndex.column() == 1):
            ok = self.setStrData("name", id, p_object);#
        if (QModelIndex.column() == 2):
            ok = self.setStrData("description", id, p_object);
        if (QModelIndex.column() == 3) and (self.toint(p_object)>0):
            ok = self.setStrData("material", id, p_object);
        if (QModelIndex.column() == 16) and (self.toint(p_object)>0):
            ok = self.setStrData("widthline", id, p_object);
        if (QModelIndex.column() == 15):
            ok = self.setStrData("colorline", id, p_object);
            print str(id)+"colorline"
        if (QModelIndex.column() == 14):
            ok = self.setStrData("typeline", id, p_object);
        if (QModelIndex.column() == 11):
            ok = self.setStrData("inangle", id, p_object);
        if (QModelIndex.column() == 10):
            ok = self.setStrData("ypos", id, p_object);
        if (QModelIndex.column() == 9):
            ok = self.setStrData("xpos", id, p_object);
        if (QModelIndex.column() == 6) and (self.toint(p_object)>0):
            ok = self.setStrData("brord", id, p_object);
        #} else {
        ...
        return super(latchesRelSqlModel, self).setData(QModelIndex, p_object, int_role)
    ...
    def setStrData(self, field_name, objId, strData):
        ...
        setQ.prepare("update latches set "+field_name+" = :val where id = :id")
